 ...
 * @since 2.0
 */
class ModuleStor extends Module
{

    protected $oMapperStor = null;
     ...
     */
    const CACHE_FIELD_DATA_PREFIX = 'stor_field_data_';
    ...
    {
        $this->oMapperStor = Engine::GetMapper(__CLASS__);
    }
    ...
    protected function SetFieldOne($sKey, $sValue, $sInstance = self::DEFAULT_INSTANCE)
    {
         ...
         */
        $this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG, array('stor_field_data'));
        /*
         * добавить запись в хранилище для ключа
         */
        return $this->oMapperStor->SetData($sKey, $sValue, $sInstance);
    }
    ...
    protected function GetFieldOne($sKey, $sInstance = self::DEFAULT_INSTANCE)
    {
        ...
        if (($mData = $this->Cache_Get($sCacheKey)) === false) {
             ...
             */
            $sWhere = $this->oMapperStor->BuildFilter(array(
                'key'      => $sKey,
                'instance' => $sInstance
            ));
            $mData = null;
             ...
             * получить данные
             */
            $aResult = $this->oMapperStor->GetData($sWhere, 1, 1);
            /*
             * есть ли данные
             */
            if ($aResult['count'] != 0) {
                $mData = $aResult['collection']['value'];
                $this->Cache_Set($mData, $sCacheKey, array('stor_field_data'), 60 * 60 * 24 * 365);    // 1 год
            }
        }
        return $mData;
    }
    ...
    protected function DeleteFieldOne($sKey, $sInstance = self::DEFAULT_INSTANCE)
    {
         ...
         * удалить данные
         */
        return $this->oMapperStor->DeleteData($sWhere, 1);
    }
    ...
    protected function GetFieldsAll($sInstance = self::DEFAULT_INSTANCE)
    {
        ...
        if (($mData = $this->Cache_Get($sCacheKey)) === false) {
             ...
             * получить данные
             */
            $mData = $this->oMapperStor->GetData($sWhere);
            $this->Cache_Set($mData, $sCacheKey, array('stor_field_data'), 60 * 60 * 24 * 365);    // 1 год
        }
        return $mData;
    }
    ...
    protected function PrepareParamValueBeforeSaving($mValue)
    {
        if (is_resource($mValue)) {
            throw new Exception('Stor: your data must be scalar value, not resource!');
        }
        return $mValue;
    }
